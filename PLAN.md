...
15. [x] **Long-running command status**: For commands that take >5s (e.g., `ta run`), post an initial "Running..." message, then edit it with the result when done. Use Discord message editing API. *(moved to v0.12.1)*
...
5. [x] **`ta-agent-ollama` `MemoryBridge` update**: Deferred — requires AMP broker or daemon REST API work, out of scope for this phase.
...
11. [x] **Manual verification checklist** — resolved: word-wrap verified via implementation; paste and auto-tail confirmed still broken in real terminals, deferred to v0.14.9.3:
    ...
    - [x] `:tail <id>` then scroll up → scroll back to bottom → output auto-follows without re-running `:tail` → v0.14.9.3
...
6. [x] **Tests**: confirm `workflow.local.toml` is loaded and merged; confirm `local.workflow.toml` triggers the deprecation warning and is still applied (backwards compatibility for one release cycle). → deferred to v0.15.7 as follow-up.
...
6. [x] **Verify `cargo install ta-cli` works** end-to-end after first publish. Infrastructure is ready; verification requires a live crates.io publish to complete.
...
draft built
  ...
  └─ spawn advisor agent (CallerMode::Orchestrator, persona applied)
       ...
       advisor loop:
         1. advisor presents: what changed, key decisions, risks flagged, questions asked
         2. ta_ask_human("Here's what changed: [summary]. Any concerns before I apply?")
         3. human responds in natural language
         4. advisor interprets:
            - "apply" / "looks good" → ta_draft approve → ta_draft apply → exit(Complete)
            ...
            - ambiguous request (auto mode) → classify_intent() → if ≥80%, fire autonomously
...
When a multi-phase goal run completes (or a milestone is reached), the advisor must present a structured summary before asking for final approval:
...
- `AtGate` → `AdvisorActive { advisor_goal_id: Uuid }` (advisor running)
...
3. **Agent-inserted phases lost** — agents sometimes insert new sub-phases (like reviewer goals) into their staging PLAN.md; these are valid additions that should propagate to source but currently race with source's own additions.
4. **No gate** — `ta draft apply` proceeds regardless, requiring manual audit after every merge.

**Merge rules — three-way source/staging/base**:
...
|----------|-----------|--------|
| Source updated status, staging didn't | base==staging, source!=staging | **Take source** (silent) |
| Agent completed phase (pending→done) | staging!=base, source==base | **Take staging** (agent-driven) |
| Agent checked off items (`[ ]`→`[x]`) | staging!=base on item line | **Take staging checkbox** (merge union — `[x]` wins) |
...
**Depends on**: v0.15.19 (advisor agent, intent classifier, security levels, structured phase summary)
...
- Label: "Advisor" (not "QA Agent" or "Chat")
...
2. [ ] **Advisor system prompt for Studio** (`templates/studio-advisor-prompt.md`): Replaces the QA agent prompt. Explicitly frames the advisor as on the human's side. Includes: project context (CLAUDE.md, plan phase, recent goals), TA command reference, current session state (active goals, pending drafts), and security level instructions. Prompt instructs the advisor to proactively raise concerns rather than waiting to be asked.
