...
| v0.13.9.1 | Baseline VCS-agnostic ignore + project type templates (earlier sub-phase of v0.13.9) |
...
21. [ ] Core tool set: bash_exec, file_read, file_write, file_list, web_fetch, memory_read, memory_write, memory_search
22. [ ] Startup: read context from `--context-file` or `$TA_GOAL_CONTEXT`, include in system prompt; emit sentinel to stderr
23. [ ] Model validation: probe `/v1/models` + test function-calling call on startup; emit clear error if model doesn't support tools
24. [ ] Graceful degradation: if model has no function calling, fall back to CoT-with-parsing mode (best-effort) with a warning
...
32. [ ] `ta agent publish <path>` — validate + submit to registry
...
# VCS-agnostic ignore: TA tells the active VCS plugin which paths to exclude.
# The git plugin writes .gitignore, Perforce writes .p4ignore, etc.
# track = explicit opt-in (these would otherwise be ignored by default)
[vcs.ignore]
always = [
  ".ta/goals/",
  ".ta/staging/",
...
The `[vcs.ignore]` section is VCS-agnostic: TA passes patterns to the active VCS plugin via a `SetIgnorePatterns` protocol message. The plugin translates them to the correct mechanism (`.gitignore`, `.p4ignore`, etc.). See VCS plugin protocol additions in item 4 below.

**Project type templates** — `constitution.toml` is generated from a YAML template matched to the project type. Templates ship as embedded YAML files (not hardcoded Rust), so they can be extended without recompiling:

```
.ta/templates/
  rust-workspace.yaml
  typescript-monorepo.yaml
  python-ml.yaml
  go-service.yaml
  generic.yaml
```

Each template YAML populates the `[[validate]]` commands, `[vcs.ignore]` patterns, and `[scan]` settings appropriate for that stack. Users can start from a template and customize, or extend with `extends = "rust-workspace"`.
...
1. [ ] **`constitution.toml` schema**: Define and document the config format. Ship TA's own rules as the default template (generated by `ta init constitution`).
   - `[[validate]]` arrays replace TA's hardcoded `[verify]` in `office.yaml`. Projects define what "passing" means — Rust adds clippy/test, TypeScript adds tsc/jest, etc.
   - `on_failure = "ask_follow_up"` emits `ValidationFailed`; auto-follow-up behaviour in v0.13.1 items 24–29.
2. [ ] **`ta init constitution`**: Scaffolding command. Detects project type, selects matching YAML template, writes `.ta/constitution.toml`. For new projects: prompts to pick type. For existing projects: auto-detects from lockfiles/config files (Cargo.toml → rust-workspace, package.json → typescript-monorepo, etc.).
3. [ ] **YAML template files**: Replace hardcoded templates in `init.rs` with embedded YAML template files, one per project type. Each defines `[[validate]]`, `[vcs.ignore]`, `[scan]`, and `[rules]` sections for that stack. Templates are loadable at runtime so power users can supply custom templates.
4. [ ] **VCS plugin protocol: `SetIgnorePatterns`**: Extend the JSON-over-stdio VCS plugin protocol with a `set_ignore_patterns` RPC call. Input: `{ always: [...], track: [...] }`. The git plugin writes/updates `.gitignore`; Perforce plugin updates `.p4ignore`; etc. Called by `ta init` and whenever `constitution.toml` `[vcs.ignore]` changes. TA never writes VCS-specific ignore files directly.
5. [ ] **Draft-time scanner reads `constitution.toml`**: Move the hardcoded §4 pattern scan (v0.11.5 item 8) to read inject/restore function names from `constitution.toml`. Projects with different conventions get correct scanning.
6. [ ] **Release pipeline reads `checklist_gate`**: The release checklist gate step (v0.11.4.4 item 9) is enabled/disabled by `constitution.toml`. The checklist content is generated from the declared rules, not hardcoded.
7. [ ] **Parallel agent review during release**: When `agent_review = true`, the release pipeline fans out two agents: the existing release notes writer, and a lighter constitution reviewer. Its output appends as a "Constitution Review" section.
8. [ ] **`ta constitution check`**: CLI command to run the scan outside of draft build. Exit code 0 = clean, 1 = violations. `--json` for machine-readable output (CI integration).
9. [ ] **Inheritance**: `constitution.toml` can `extends = "ta-default"` or `extends = "rust-workspace"` to inherit a built-in profile and override specific sections only.
10. [ ] **Documentation**: "How to write a constitution for your project" guide in `docs/`. Includes worked example for a web service with DB migration injection patterns.

**Files**: `.ta/constitution.toml` (new), `.ta/templates/*.yaml` (new), `crates/ta-submit/src/vcs_plugin_protocol.rs` (new `SetIgnorePatterns` message), `apps/ta-cli/src/commands/` (init, check, draft build scan, release step), `crates/ta-workspace/src/` (scanner).

#### Version: `0.13.9-alpha`

---

### v0.13.9.1 — Baseline VCS-Agnostic Ignore & Project Init Templates
<!-- status: pending -->
**Goal**: Ship the minimum viable version of the `[vcs.ignore]` mechanism and project type templates from v0.13.9 as an earlier sub-phase, so projects can manage ignore patterns through TA before the full constitution scanner is ready.

**Depends on**: v0.12.0.2 (VCS Externalization — plugin protocol exists), v0.13.9 design

#### Items

1. [ ] **`SetIgnorePatterns` VCS plugin message**: Add to protocol (`crates/ta-submit/src/vcs_plugin_protocol.rs`). Git plugin (`plugins/ta-vcs-git/`) implements it by writing/updating `.gitignore`. Other plugins get a no-op default until they implement it.
2. [ ] **`ta init` writes `[vcs.ignore]`**: `ta init` generates a minimal `constitution.toml` with just the `[vcs.ignore]` section (always/track lists). Calls `SetIgnorePatterns` on the active VCS plugin immediately.
3. [ ] **Project type template YAML**: Ship at least `rust-workspace.yaml` and `generic.yaml` as embedded templates. `ta init` picks one based on detection (Cargo.toml → rust-workspace, else generic).
4. [ ] **`ta init --template <name>`**: Allow manual template selection. `ta init --list-templates` shows available options.
5. [ ] **`ta constitution sync-ignore`**: Re-apply `[vcs.ignore]` patterns to the VCS plugin (idempotent). Run after manually editing `constitution.toml` or after upgrading TA.
...
#### Shell Input & Scroll UX Polish

9. [ ] **Input line-wrap (growing prompt)**: The `ta>` prompt input grows vertically as the user types or pastes content that exceeds the terminal width. Each wrapped line adds a new input row, shrinking the scrollable output area above by the same amount. Resizing the window recalculates input height. Behaviour mirrors a textarea, not a single-line input.
   - Implementation: track cursor position + terminal width; on each keystroke/paste, compute required input rows = `ceil(content_length / width)`; resize the input pane, re-render output pane to fill remaining height.
10. [ ] **Auto-scroll anchor with bottom padding**: The scrollable output window always renders one blank line at the bottom when the user is anchored at the end (tailing mode). The blank line is a visual indicator: "you are seeing the latest output." When the user scrolls up (breaks anchor), the blank line disappears. Scrolling back to the bottom re-anchors.
11. [ ] **Window resize reflows both panes**: On `SIGWINCH` (terminal resize), recompute: new terminal dimensions → input height from current input content → output height = total − input − status bar. Scroll position in the output pane adjusts so the anchor is maintained if the user was at the bottom.
