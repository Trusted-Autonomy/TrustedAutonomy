...
1. [ ] **Write manifest in `ta init`**: After writing `.gitignore` and `.taignore`, call `write_ide_excludes_manifest(project_root)` which serializes `ta_runtime_dirs()` to `.ta/ide-excludes.json`. Idempotent — rewrite unconditionally (file is generated, not user-edited).
2. [ ] **`ta doctor` stale-manifest check**: Compare `dirs` in existing `.ta/ide-excludes.json` against current `ta_runtime_dirs()`. If any dir is missing, emit `[warn] .ta/ide-excludes.json is out of date — run ta doctor --fix`. `--fix` rewrites the file.
3. [ ] **Add `.ta/ide-excludes.json` to `.gitignore`** (and verify it is in `.taignore` so it's excluded from staging diffs).
4. [ ] **JetBrains plugin manifest read**: In `TaDirectoryIndexExcludePolicy.kt`, attempt to read `.ta/ide-excludes.json` at call time. Parse `dirs`, resolve each against the `.ta/` directory, filter to existing directories, return as exclude URLs. Keep the hardcoded companion object as a fallback for projects without a manifest.
5. [ ] **Tests**: `manifest_written_on_init`, `manifest_contains_all_runtime_dirs`, `doctor_detects_stale_manifest`, `doctor_fix_rewrites_manifest`, `jetbrains_policy_reads_manifest` (mock project root with JSON file), `jetbrains_policy_falls_back_to_hardcoded` (no JSON file present).
6. [ ] **USAGE.md**: Add "Community editor plugins" subsection to the "IDE integration" section. Document the `.ta/ide-excludes.json` contract. Link to the JetBrains plugin source as a reference implementation. Note that `ta init` / `ta doctor --fix` regenerate the file after TA upgrades.
