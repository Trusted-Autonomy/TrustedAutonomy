...
pub const LOCAL_TA_PATHS: &[&str] = &[
    ...
    "wiki-ingestion-state.json", // VCS-connector's last-reviewed-sha checkpoint (ta-virtual-team sub-project 2): machine-local, not shared state
    // Runtime files
    ...
    "whiteboard-session.json", // per-goal team-session whiteboard token, written into staging by `ta run --team-session-id` (v0.17.11.8) — a live secret, must never be committed
