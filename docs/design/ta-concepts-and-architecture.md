...
- **AgenC** (agenc.tech) — real, but a different problem space: a Solana-based agent marketplace toolkit (escrowed gig work between agents, on-chain identity/capability bitmasks). Structurally validates TA's core thesis, though — its "preview-first execution, policy-gated mutation tools, human approval before settlement" is a genuine parallel to TA's staging→Commit gate, just applied to payments/escrow instead of internal engineering goals. **Correction (2026-07-05): user meant a different "AgenC" than this one** — not re-identified yet; re-verify against a supplied link before citing further.
- **Alembic vs. Alembiq — corrected 2026-07-05**: user meant **Alembiq** (alembiq.app), not Alembic. Verified via fetch: Alembiq is a $15/mo commercial Claude "skills" subscription product — a codified, reusable instruction/knowledge-base library (27 ready-made "skills") with a self-improving meta-layer (a "Prompt Engineer" skill, a self-audit/refine skill). It does not implement triggers, routing, or back-office execution at all — not a 3-tier orchestration comparable. The one real parallel: its "skill" concept (reusable, community-buildable, codified knowledge unit) resembles what TA's `PersonaConfig`/constitution-extension should become, not an architecture match.
...
### 13.1 Repo Organization — Monorepo + Library Planes + Data-Format Specs (2026-07-05)

User's actual goal, stated directly: TA should accelerate their own work, but setup/reliability friction is what's currently blocking that — so the organizational answer needs to optimize for less setup surface, not architectural purity for its own sake. Question raised: should TA split into modules/repos for the 3 tiers + reusable libraries + data-format specs, with Studio as an add-on?

**Recommendation: stay monorepo (single Cargo workspace). Do not split into separate repos.** Multi-repo only pays off when pieces need independent release cadence or separate team ownership — neither applies (solo user, single release train) — and a split would *add* exactly the kind of friction already causing pain (cross-repo schema drift, version-pinning, more install/setup steps), not remove it. TA is already workspace-organized (~30 crates); the fix is tighter internal boundaries, not repo boundaries.

**Organize the three tiers as clean library crates, decoupled from any binary**, so the "Brain" is genuinely reusable rather than reimplemented per entry point:
- `ta-intake` (Triggers) — owns only "an event of type X arrived, here's its normalized payload." Per-type configs are data (`.ta/triggers/<type>.toml`), same pattern as personas/plugins (§1.6) — this is the mechanism for "community can create/improve per-type pre-built configs."
- `ta-brain` (routing/orchestration) — the §3 mapping tree as a pure function: `(explicit goal request | normalized intake event) -> {team, persona, agent, security_tier, priority}`. Discipline: **one decision function**, called identically whether work originates from an explicit `ta run` or a fired trigger — that's what makes it reusable across orchestration+routing and back-office supervision, matching the user's own framing.
- Back office — already the most mature code (`ta-changeset`/`ta-policy`/`ta-goal`/`ta-submit`); needs consolidation per the already-queued v0.17.0.12.14/12.15 work, not new invention.

**Data format specs are the real interface boundary, not repo splits.** Publish versioned schemas (e.g. `schemars`-derived JSON Schema) for Goal, Draft/Artifact, TriggerEvent, RoutingDecision, Persona. This is what lets Studio, community trigger-configs, and community plugins build against TA without compiling Rust — the spec is the contract.
