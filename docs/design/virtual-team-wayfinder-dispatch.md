...
4. ~~Repo name and org~~ **Resolved**: `ta-virtual-team` under the `Trusted-Autonomy` GitHub org, private (2026-08-26). Product/marketing branding for the virtual-team/office/business tiers is a separate, ongoing naming exercise — see the branding note below; the technical repo name (`ta-virtual-team`) is independent of whatever consumer-facing product name ships under it, the same way `ta-agent-whiteboard` (crate name) and "Amplified Office" (product name) already coexist.
