...
#### Community editor plugins (Zed, Helix, Neovim, Nova, Cursor, and others)

`ta init` writes `.ta/ide-excludes.json` — a machine-readable manifest that lists every TA runtime directory that should be excluded from file search and symbol indexing:

```json
{
  "version": 1,
  "ta_dir": ".ta",
  "dirs": ["staging/", "goals/", "sessions/", "store/", "backups/", "memory/",
           "events/", "interactions/", "pr_packages/", "review/", "heartbeats/",
           "workflow-runs/", "advisor-notes/", "draft-build-ctx/", "link-cache/"]
}
```

**To add TA index exclusion support for any editor**: read `.ta/ide-excludes.json`. The `dirs` array lists paths relative to the `.ta/` directory that should be excluded from file search and symbol indexing. The file is regenerated by `ta init` and `ta doctor --fix`, so it stays current after TA upgrades — no fork or Rust knowledge required.

The JetBrains plugin is the reference implementation: it reads this file at IDE startup and falls back to a hardcoded list for projects that have not yet been re-initialized. See [`plugins/jetbrains-ta/src/main/kotlin/com/trustedautonomy/ta/TaDirectoryIndexExcludePolicy.kt`](../plugins/jetbrains-ta/src/main/kotlin/com/trustedautonomy/ta/TaDirectoryIndexExcludePolicy.kt).

The manifest is machine-local and gitignored (`ta setup vcs` adds `.ta/ide-excludes.json` to `.gitignore`). Run `ta doctor --fix` after upgrading TA to refresh it:

```
[warn] IDE manifest  .ta/ide-excludes.json is out of date — 1 dir(s) missing: link-cache/
       run `ta doctor --fix` to regenerate the manifest
```
