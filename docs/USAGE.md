...
> **Perforce users**: add `.ta/staging/`, `.ta/goals/`, `.ta/events/`, `.ta/sessions/`, `.ta/daemon.toml`, `.ta/local.workflow.toml`, `.mcp.json`, and `ONBOARDING.md` to your `.p4ignore`. Commit `.ta/workflow.toml` and `.ta/policy.yaml` — these are shared team config. See [Perforce: shared vs personal config](#perforce-shared-vs-personal-config) for details.
...
[submit.perforce]
# workspace = "my-workspace"  # personal — see "Shared vs personal config" below
shelve_by_default = true       # Shelve instead of submit to depot (team default)
...
#### Perforce: shared vs personal config

`workflow.toml` is committed to your depot so your whole team shares the same VCS settings. However, the **P4 workspace/client name** is personal — it varies per developer and must not be committed.

**Committed to depot** (`.ta/workflow.toml`):
```toml
[submit]
adapter = "perforce"
auto_submit = true
...
[submit.perforce]
shelve_by_default = true   # team default: shelve, don't submit directly
```

**Personal, not committed** (`.ta/local.workflow.toml` — add to your `.p4ignore`):
```toml
[submit.perforce]
workspace = "michael_ue5_ws"   # your personal P4 workspace/client name
```

`local.workflow.toml` deep-merges on top of `workflow.toml` at load time (local values win). It is not yet automatically generated by `ta setup` — create it manually and add it to your `.p4ignore`.

**P4PORT, P4CLIENT, P4USER**: These come from env vars or your `.p4config` file (standard Perforce convention). TA reads them from the environment — do not add them to `workflow.toml`.

**Retroactive VCS setup** (if `ta init` did not detect Perforce): manually add the `[submit]` block above to `.ta/workflow.toml`. `ta setup vcs` for guided re-configuration is coming in v0.13.13.
