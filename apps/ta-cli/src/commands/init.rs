...
fn run_init(
    ...
    // Security level prompt (v0.15.14.4).
    let security_level: &str = if interactive {
        let answer = prompt(
            "Security level? [low] solo-dev / mid team / high regulated",
            "low",
        );
...
"#
    ...
    // Security section (only written if non-default to keep low-level files minimal).
    let security_section = match security_level {
        "mid" => {
            r#"
# Security level profile (v0.15.14.4)
# "mid" enables process sandbox, forbidden bash patterns, and hash-chained audit log.
[security]
level = "mid"
"#
        }
        "high" => {
            r#"
# Security level profile (v0.15.14.4)
# "high" enables sandbox, approval gate, HMAC-signed audit chain, and no WebSearch.
# Disabling sandbox or scan requires an explicit override (logged as a warning).
[security]
level = "high"

# [security.secrets]
# scan = "warn"   # downgrade from "block" (default for high) if needed
...
"#
        ...
        _ => {
            r#"
# Security level profile (v0.15.14.4) — default is "low" (solo-dev, no extra restrictions).
# Set to "mid" for teams or "high" for regulated environments:
# [security]
# level = "mid"
"#
        }
...
{}{}"#,
        verify_section, security_section
    );
    std::fs::write(&path, content)?;
...
"#;
    ...
    fn rust_template_workflow_has_analysis_section() {
        ...
        let ta_dir = dir.path().join(".ta");
        std::fs::create_dir_all(&ta_dir).unwrap();
        generate_workflow_toml(&ta_dir, &ProjectType::RustWorkspace, "low").unwrap();
    ...
    fn python_template_workflow_has_analysis_section() {
        ...
        let ta_dir = dir.path().join(".ta");
        std::fs::create_dir_all(&ta_dir).unwrap();
        generate_workflow_toml(&ta_dir, &ProjectType::Python, "low").unwrap();
        let content = std::fs::read_to_string(ta_dir.join("workflow.toml")).unwrap();
        assert!(
    ...
    fn typescript_template_workflow_has_analysis_section() {
        ...
        let ta_dir = dir.path().join(".ta");
        std::fs::create_dir_all(&ta_dir).unwrap();
        generate_workflow_toml(&ta_dir, &ProjectType::TypeScript, "low").unwrap();
        let content = std::fs::read_to_string(ta_dir.join("workflow.toml")).unwrap();
        assert!(
    ...
    fn go_template_workflow_has_analysis_section() {
        ...
        let ta_dir = dir.path().join(".ta");
        std::fs::create_dir_all(&ta_dir).unwrap();
        generate_workflow_toml(&ta_dir, &ProjectType::Go, "low").unwrap();
        let content = std::fs::read_to_string(ta_dir.join("workflow.toml")).unwrap();
        assert!(
