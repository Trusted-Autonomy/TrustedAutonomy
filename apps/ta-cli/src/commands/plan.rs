...
// ── Data model + schema-driven parsing (v0.17.11.1: extracted to ta-plan) ──
pub use ta_plan::{
    load_plan, parse_plan, parse_plan_with_schema, phase_ids_match, update_phase_status,
    update_phase_status_with_schema, PhasePattern, PlanPhase, PlanSchema, PlanStatus,
};
// Only used by this module's own tests (constructing a custom PlanSchema
// without going through PlanSchema::default_schema()) — cfg-gated so a
// non-test build doesn't warn about an unused import.
#[cfg(test)]
use ta_plan::{default_doc_search_dirs, default_statuses};
...
// v0.17.11.1: extracted to ta-plan.
pub use ta_plan::{find_in_progress, find_next_pending};
// v0.17.11.1: extracted to ta-plan.
pub use ta_plan::{mark_phase_in_source, record_history};

/// Reset a phase from `in_progress` back to `pending` in the source PLAN.md,
/// then best-effort release the daemon's in-memory claim over HTTP so the
/// next `ta run` doesn't need a daemon restart. Thin wrapper: the core
/// reset+history-log logic lives in `ta_plan::reset_phase_if_in_progress`;
/// the daemon-release HTTP call is CLI-process-specific and stays here.
pub fn reset_phase_if_in_progress(
    project_root: &Path,
    phase_id: &str,
    note: &str,
) -> anyhow::Result<bool> {
    let did_reset = ta_plan::reset_phase_if_in_progress(project_root, phase_id, note)?;
    if did_reset {
...
fn detect_schema_from_content(content: &str, source: &str) -> PlanSchema {
    ...
    // Fallback: generic ## heading pattern. `statuses`/`doc_search_dirs` come
    // from the default schema via struct-update syntax rather than calling
    // ta-plan's default_statuses()/default_doc_search_dirs() directly, to
    // avoid importing them outside test code (see the #[cfg(test)] import
    // above).
    PlanSchema {
...
// v0.17.11.1: extracted to ta-plan.
pub use ta_plan::{binary_version, collect_dependency_warnings, is_sub_phase, parent_phase_id};
...
// v0.17.11.1: extracted to ta-plan.
pub use ta_plan::{
    candidate_waves, insert_adhoc_phase, last_completed_phase_id, next_actionable_phase_id,
    next_actionable_phases,
};
